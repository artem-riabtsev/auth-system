...
from .models import AccessRule, Role, BusinessElement, PermissionType
...
class PermissionChecker:
    ...
    def get_user_permissions(user):
        ...
        for role in roles:
            rules = AccessRule.objects.filter(role=role).select_related('element', 'permission_type')
            for rule in rules:
                ...
                if rule.scope == 'ALL' or (rule.scope == 'OWN' and current_scope != 'ALL'):
